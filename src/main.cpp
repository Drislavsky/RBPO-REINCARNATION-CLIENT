...
namespace {
    ...
    void ScheduleTrayRetry() {
        ...
        if (g_tray_retry_count >= kMaxTrayRetryCount) {
            WriteClientDebugLog(L"tray icon retry limit reached; tray icon was not added");
            return;
        }
    ...
    bool AddTrayIcon() {
        ...
        if (!Shell_NotifyIconW(NIM_ADD, &g_tray_data)) {
            ...
            StringCchPrintfW(
                text,
                ARRAYSIZE(text),
                L"Shell_NotifyIconW(NIM_ADD) failed. GetLastError = %lu. Retrying silently.",
                error);
            WriteClientDebugLog(text);
